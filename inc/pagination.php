 ...
 */
function cf_pagination( $args = [], $class = 'pagination' ) {
	...
	$args = wp_parse_args(
		...
		[
			'mid_size'           => 2,
			'prev_next'          => true,
			'prev_text'          => __( '&laquo;', 'freedomtheme' ),
			'next_text'          => __( '&raquo;', 'freedomtheme' ),
			'type'               => 'array',
			'current'            => max( 1, get_query_var( 'paged' ) ),
			'screen_reader_text' => __( 'Posts navigation', 'freedomtheme' ),
		]
	);
	...
	<nav aria-labelledby="posts-nav-label">
		...
		<h2 id="posts-nav-label" class="sr-only">
			<?php print esc_html( $args['screen_reader_text'] ); ?>
		</h2>

		<ul class="<?php print esc_attr( $class ); ?>">

			<?php
			foreach ( $links as $key => $link ) {
				?>
				<li class="page-item <?php print strpos( $link, 'current' ) ? 'active' : ''; ?>">
					<?php print str_replace( 'page-numbers', 'page-link', $link ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</li>
				<?php
			}
			?>
